...
if [[ "$ROFI_RETV" == "0" ]]; then
    # List mode - show colors in RGB format
    ...
    for color_name in "Background" "Foreground" "Cursor" "Color 0" "Color 1" "Color 2" "Color 3" "Color 4" "Color 5" "Color 6" "Color 7"; do
        ...
        # Format label: "Color Name    rgb(r, g, b)"
    ...
    # Selection mode - copy RGB value to clipboard
    read -r selected

    # Extract RGB value
    if [[ "$selected" =~ rgb\(([0-9,\ ]+)\) ]]; then
        rgb_value="rgb(${BASH_REMATCH[1]})"
        echo -n "$rgb_value" | wl-copy
    fi
fi
