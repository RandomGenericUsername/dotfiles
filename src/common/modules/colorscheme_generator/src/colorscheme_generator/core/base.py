...
class ColorSchemeGenerator(ABC):
    ...
    All backend implementations (PywalGenerator, WallustGenerator, CustomGenerator)
    must inherit from this class and implement its abstract methods.
    
    The generator's responsibility is to extract colors from an image and return
    a ColorScheme object. It does NOT write output files - that's the job of
    OutputManager.
    ...
    def generate(self, image_path: Path, config: GeneratorConfig) -> ColorScheme:
        ...
        This method extracts colors from the given image and returns a ColorScheme
        object containing the color data. It should NOT write any output files.
    ...
    def ensure_available(self) -> None:
        ...
        if not self.is_available():
            raise BackendNotAvailableError(
                self.backend_name, f"{self.backend_name} is not installed or not in PATH"
            )
